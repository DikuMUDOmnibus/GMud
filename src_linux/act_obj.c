...
void do_eat (CHAR_DATA * ch, char *argument)
{
	...
	switch (obj->item_type) 
	{
		...
		if (obj->value[3] != 0) {
			...
			af.duration = (short) (2 * obj->value[0]);
			af.location = APPLY_NONE;
			af.modifier = 0;
			af.bitvector = AFF_POISON;
			affect_join (ch, &af);
...
{
	{98, 98, 02, 02, (long *) &ch->pcdata->xcarry,	&nullvalue, 
		"{wYou have gained the ability to carry {c%d{w more items{x\n\r"	 	},
	{99, 99, 10, 10, (long *) &ch->pcdata->xweight,	&nullvalue,
		"{wYou have gained the ability to carry {c%ld{wlbs more weight{x\n\r" 	},
	{97, 97, 02, 03, (long *) &ch->practice, 		&nullvalue,
		"{wYou have gained {c%d{w practice sessions{x\n\r"						},
	{96, 96, 01, 02, (long *) &ch->train, 		&nullvalue,
		"{wYou have gained {c%d{w training sessions{x\n\r" 						},
	{76, 80, 01, 30, (long *) &ch->hit, 		&nullvalue,
		"{wYou have gained {c%d{w hit points{x\n\r"								},
	{81, 85, 01, 30, (long *) &ch->mana, 		&nullvalue,	
		"{wYou have gained {c%d{w mana points{x\n\r"			 				},
	{86, 90, 01, 30, (long *) &ch->move, 		&nullvalue,
		"{wYou have gained {c%d{w movement points{x\n\r"			 			},
	{91, 91, 01, 01, (long *) &ch->pcdata->perm_hit, (long *) &ch->max_hit,	
		"{wYou have gained {c%d{w permanent hit point{x\n\r"		 			},
	{93, 93, 01, 01, (long *) &ch->pcdata->perm_mana, (long *) &ch->max_mana, 
		"{wYou have gained {c%d{w permanent mana point{x\n\r" 					},
	{94, 94, 01, 01, (long *) &ch->pcdata->perm_move, (long *) &ch->max_move,
	...
	if (arg[0] == '\0' || !str_cmp (arg, ch->name)) {
		sprintf(buf2, "$n offers $mself to %s, who graciously declines.", 
...
/*	    if (!str_cmp("all",arg) || !str_prefix("all.",arg))
	...
	ITEM_CORPSE_PC)
	            ...
	            found = TRUE;
	            sprintf(buf, "{c%s gives you {Y%d{c silver for your sacrifice	of {w%s{c.{x\n\r", 
					deity_table[ch->pcdata->deity].name, silver, obj->short_descr );
				send_to_char(buf, ch);
				sprintf(buf2, "$n sacrifices $p to %s.",
					deity_table[ch->pcdata->deity].name );
   ...
   if (!str_cmp("all",arg) || !str_prefix("all.",arg))
   {
	 ...
	 if (obj->item_type != ITEM_CORPSE_NPC && obj->item_type !=
      ...
      if (found)
      {
         ch->silver += total_money;
	     sprintf(buf, 
             "{W%s{c gives you {W%d{c silver coins for sacrifice of {w%d{c item(s).{x\n\r",
             deity_table[ch->pcdata->deity].name,total_money, count );
         send_to_char(buf, ch);
         sprintf(buf, "{C$n{c sacrifices {w%d{c items to {W%s{c.{x\n\r", count, deity_table[ch->pcdata->deity].name);
	 ...
	 { 
	    ...
	    for (gch = ch->in_room->people;gch;gch = gch->next_in_room)
	       if (is_same_group(ch,gch))
	          members++;
	    if (members > 1 && total_money > 1)
	    {
	       sprintf(buf,"%d",total_money);
	       do_split(ch,buffer);
	    }
        }
      ...
      {
         send_to_char("{wThere is nothing to sacrifice in this room.{x\n\r",ch);
      }
      return;
   }
	...
	if (silver < 1)
		printf_to_char
			(ch, "%s gives you nothing for your sacrifice.\n\r",
			deity_table[ch->pcdata->deity].name );
	else if (silver == 1)
			printf_to_char
				(ch, "%s gives you one silver coin for your sacrifice.\n\r",
				deity_table[ch->pcdata->deity].name );
		else 
			printf_to_char (ch,
					 "%s gives you %d silver coins for your sacrifice.\n\r",
					 deity_table[ch->pcdata->deity].name , silver);
...
CHAR_DATA *find_keeper (CHAR_DATA * ch)
{
	...
	sprintf( buf, "{RSorry, {GI am closed now. Come back between {W%d%s {Gand {W%d%s {Gwhen i am open{x",
		(pShop->open_hour % 12 == 0) ? 12 : pShop->open_hour % 12, 
		pShop->open_hour >= 12 ? "pm" : "am",
		(pShop->close_hour % 12 == 0) ? 12 : pShop->close_hour % 12, 
		pShop->close_hour >= 12 ? "pm" : "am");
	/*
	 * Undesirables.
	 *
...
void do_list (CHAR_DATA * ch, char *argument)
{
	...
	if (IS_SET (ch->in_room->room_flags, ROOM_PET_SHOP)) {
		...
		for (pet = pRoomIndexNext->people; pet; pet = pet->next_in_room) {
			if (IS_SET (pet->act, ACT_PET) && IS_NPC(pet)) /*IS_NPC 052301 */
			{
				if (!found) 
				{
					found = TRUE;
