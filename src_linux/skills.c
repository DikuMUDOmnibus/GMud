 ...
 * colorized and viewing improvements made by TAKA
 */
...
void do_spells(CHAR_DATA *ch, char *argument)
{
    BUFFER *buffer;
    char arg[MAX_INPUT_LENGTH];
    char spell_columns[LEVEL_MAX_HERO + 9];
    char spell_list[LEVEL_MAX_HERO + 9][MSL/2];
 	...
 	if (str_prefix(argument,"all"))
	{
	    ...
	    if (!is_number(arg))
	    {
		send_to_char("{RArguments must be numerical or all.{x\n\r",ch);
		return;
	    }
	    max_lev = atoi(arg);
	    ...
	    {
		sprintf(buf,"{RLevels must be between {W1 {Rand {W%d{R.{x\n\r",LEVEL_HERO);
		send_to_char(buf,ch);
		return;
	    }
		...
		{
		    sprintf(buf,
			"{RLevels must be between {W1 {Rand {W%d{R.{x\n\r",LEVEL_HERO);
		    send_to_char(buf,ch);
		    return;
		}
		...
		if (min_lev > max_lev)
		{
		    send_to_char("{RThat would be silly.{x\n\r",ch);
		    return;
		}
	    }
	}
	...
	for (level = 0; level < max_lev; level++)
    {
...
/*
 * colored by TAKA and improved layout by TAKA
 */
...
void do_skills(CHAR_DATA *ch, char *argument)
{
    ...
    BUFFER *buffer;
    char arg[MAX_INPUT_LENGTH];
	char skill_list[LEVEL_HERO + 9][MSL/2];
    char skill_columns[LEVEL_HERO + 9];
    int sn, level, min_lev = 1, max_lev = LEVEL_HERO + 1;
    bool fAll = FALSE, found = FALSE;
    char buf[MAX_STRING_LENGTH];
	...
	if (str_prefix(argument,"all"))
	{
		...
		{
		    sprintf(buf,
			"{RLevels must be between {W1 {Rand {W%d.{x\n\r",LEVEL_HERO);
		    send_to_char(buf,ch);
		    return;
		}
...
int exp_per_level(CHAR_DATA *ch, int points)
{
    ...
    if (points < 40)
	return 1000 * (ch->ri->class_mult[ch->class] ?
		       ch->ri->class_mult[ch->class]/100 : 1);
    ...
    return expl * ch->ri->class_mult[ch->class]/100;
}
	...
	return;
     ...
     for (sn = 0; sn < MAX_IN_GROUP; sn++)
     {
		...
		if (!sn)
		{
			if (group_table[gn].rating[ch->class] != -1)
			{
				sprintf(buf,"{GGroup Name; {W%-20s {GCost: {M%3d{x\n\r",
					group_table[gn].name, group_table[gn].rating[ch->class]);
			}
			else
			{
				sprintf(buf,"{GGroup Name; {W%-20s {GCost: {MN/A{x\n\r",
					group_table[gn].name);
			}
			send_to_char(buf,ch);
			send_to_char("{GName                 Level Rating Min.Mana{x\n\r",ch);
		}

    	sno = skill_lookup(group_table[gn].spells[sn]);

		if (skill_table[sno].skill_level[ch->class] <= (MAX_LEVEL + 1))
		{
			sprintf(buf,"{W%-20s  {G%3d   %4d    %3d{x\n\r",
				group_table[gn].spells[sn],
				skill_table[sno].skill_level[ch->class],
				skill_table[sno].rating[ch->class],
				skill_table[sno].min_mana);
			send_to_char(buf,ch);
		}
		else
		{
			sprintf(buf,"{W%-20s  {G<group name>{x\n\r",
				group_table[gn].spells[sn]);
			send_to_char(buf,ch);
		}
