...
const char * fn_keyword[] =
{
    ...
    "carryall",		/* if carryall $n sword	- does $n have a 'sword' */
					/* if carryall $n 1233	- does $n have obj vnum 1233 */
					/* check if they carry or wear the item */
 ...
 */
bool has_item( CHAR_DATA *ch, long vnum, int item_type, bool fWear )
{
    OBJ_DATA *obj;
...
bool has_item2( CHAR_DATA *ch, long vnum, sh_int item_type, bool fWear )
{
    OBJ_DATA *obj;
 ...
 */
bool get_mob_vnum_room( CHAR_DATA *ch, OBJ_DATA *obj, ROOM_INDEX_DATA *room, long vnum )
{
    CHAR_DATA *mob;
 ...
 */
bool get_obj_vnum_room( CHAR_DATA *ch, OBJ_DATA *obj, ROOM_INDEX_DATA *room, long vnum )
{
    OBJ_DATA *vobj;
 ...
 *----------------------------------------------------------------------
 */
int cmd_eval_mob( long vnum, char *line, int check,
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
	...
	{
	    sprintf( buf, "Cmd_eval_mob: prog %ld syntax error(2) '%s'",
		vnum, original );
	    bug( buf, 0 );
	    return FALSE;
	}
    ...
    if ( buf[0] != '$' || buf[1] == '\0' )
    {
	sprintf( buf, "Cmd_eval_mob: prog %ld syntax error(3) '%s'",
		vnum, original );
	bug( buf, 0 );
        return FALSE;
    }
	...
	default:
	    sprintf( buf, "Cmd_eval_mob: prog %ld syntax error(4) '%s'",
		vnum, original );
	    bug( buf, 0 );
	    return FALSE;
    }
	...
	case CHK_CARRY_WEAR:
	    if ( is_number( buf ) )
			return( lval_char != NULL && has_item2( lval_char, atoi(buf), -1, FALSE ) );
	    else
			return( lval_char != NULL && (get_obj_carry2( lval_char, buf, lval_char ) != NULL) );
	case CHK_WEARS:
	    if ( is_number( buf ) )
		return( lval_char != NULL && has_item( lval_char, atol(buf), -1, TRUE ) );
	    else
    ...
    if ( (oper = keyword_lookup( fn_evals, buf )) < 0 )
    {
	sprintf( buf, "Cmd_eval_mob: prog %ld syntax error(5): '%s'",
		vnum, original );
	bug( buf, 0 );
	return FALSE;
    }
...
int cmd_eval_obj( long vnum, char *line, int check,
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
	...
	if ( (oper = keyword_lookup( fn_evals, buf )) < 0 )
	{
	    sprintf( buf, "Cmd_eval_obj: prog %ld syntax error(2) '%s'",
		vnum, original );
	    bug( buf, 0 );
	    return FALSE;
	}
    ...
    if ( buf[0] != '$' || buf[1] == '\0' )
    {
	sprintf( buf, "Cmd_eval_obj: prog %ld syntax error(3) '%s'",
		vnum, original );
	bug( buf, 0 );
        return FALSE;
    }
	...
	default:
	    sprintf( buf, "Cmd_eval_obj: prog %ld syntax error(4) '%s'",
		vnum, original );
	    bug( buf, 0 );
	    return FALSE;
    }
	...
	case CHK_CARRIES:
	    if ( is_number( buf ) )
		return( lval_char != NULL && has_item( lval_char, atol(buf), -1, FALSE ) );
	    else
		return( lval_char != NULL && (get_obj_carry( lval_char, buf, lval_char ) != NULL) );
	case CHK_WEARS:
	    if ( is_number( buf ) )
    ...
    if ( (oper = keyword_lookup( fn_evals, buf )) < 0 )
    {
	sprintf( buf, "Cmd_eval_obj: prog %ld syntax error(5): '%s'",
		vnum, original );
	bug( buf, 0 );
	return FALSE;
    }
...
int cmd_eval_room( long vnum, char *line, int check,
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
	...
	if ( (oper = keyword_lookup( fn_evals, buf )) < 0 )
	{
	    sprintf( buf, "Cmd_eval_room: prog %ld syntax error(2) '%s'",
		vnum, original );
	    bug( buf, 0 );
	    return FALSE;
	}
    ...
    if ( buf[0] != '$' || buf[1] == '\0' )
    {
	sprintf( buf, "Cmd_eval_room: prog %ld syntax error(3) '%s'",
		vnum, original );
	bug( buf, 0 );
        return FALSE;
    }
	...
	default:
	    sprintf( buf, "Cmd_eval_room: prog %ld syntax error(4) '%s'",
		vnum, original );
	    bug( buf, 0 );
	    return FALSE;
    }
	...
	case CHK_CARRY_WEAR:
	    if ( is_number( buf ) )
			return( lval_char != NULL && has_item2( lval_char, atol(buf), -1, FALSE ) );
	    else
			return( lval_char != NULL && (get_obj_carry2( lval_char, buf, lval_char ) != NULL) );
	case CHK_WEARS:
	    if ( is_number( buf ) )
    ...
    if ( (oper = keyword_lookup( fn_evals, buf )) < 0 )
    {
	sprintf( buf, "Cmd_eval_room: prog %ld syntax error(5): '%s'",
		vnum, original );
	bug( buf, 0 );
	return FALSE;
    }
...

{
    ...
    {
	if ( mob )
	    sprintf( bugbuf, "Progs: MAX_CALL_LEVEL exceeded, vnum %ld, mprog vnum %d",
	      mvnum, pvnum );
	else if ( obj )
	    sprintf( bugbuf, "Progs: MAX_CALL_LEVEL exceeded, vnum %ld oprog vnum %d.",
	      ovnum, pvnum );
	else
	    sprintf( bugbuf, "Progs: MAX_CALL_LEVEL exceeded, vnum %ld rprog vnum %d.",
	      rvnum, pvnum );
	bug( bugbuf, 0 );
	call_level--;
	return;
	...
	*b = *c = *d = '\0';
	...
	else if ( (!level || cond[level] == TRUE) && buf[0] != '\0' )
	{
		 ...
		 */
		if ( !mob )
		    bug( "Normal MUD command in non-MOBprog, prog vnum %ld", pvnum );
		else
		    interpret( mob, data );
	    }
	}
