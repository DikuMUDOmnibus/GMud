...
/*dos*/ char strbackup[MAX_INPUT_LENGTH+4];
    FILE *fp;
 ...
 */
 if ( !IS_VALID(ch)) {
	...
	fprintf(fp,"Lev %3d Trust %3d  %s%s\n",
    ...
    /*
	 * save remort as .rmy file so reverse remort command can be written
	 */
	if (ch->pcdata->confirm_remort)
	{
		...
		unlink( strbackup );
#endif
		rename ( strsave, strbackup );
	}

#if defined(WIN32)
/*dos*/	strcpy( strbackup, strsave );
/*dos*/	strcat( strbackup, ".bak" );
/*dos*/	_unlink( strbackup );
/*dos*/	rename ( strsave, strbackup );
#endif
...
void fwrite_char( CHAR_DATA *ch, FILE *fp )
{
   ...
   if (ch->short_descr[0] != '\0')
      	fprintf( fp, "ShD  %s~\n",	ch->short_descr	);
   if (ch->pcdata->rstmsg[0] != '\0')						/* 1129A Taka Restore Message */
      	fprintf( fp, "RstM %s~\n",	ch->pcdata->rstmsg	);	/* 1129A Taka Restore Message */
   if (ch->pcdata->whotitl[0] != '\0')						/* 1214  Taka Who Title */
      	fprintf( fp, "WhoT %s~\n",	ch->pcdata->whotitl	);	/* 1214  Taka Who Title */
    ...
    if (ch->clan)
   	{ /*1228*/
/*1228*/   	fprintf( fp, "Clan %s~\n", ch->cid->short_name);
/*1228*/   	fprintf( fp, "ClRk %d\n", ch->clan_rank);
	} /*1228*/
/*    	fprintf( fp, "Clan %s~\n",clan_table[ch->clan].name); 1228 */
    fprintf( fp, "Sex  %d\n",	ch->sex			);
    fprintf( fp, "Cla  %d\n",	ch->class		);
    fprintf( fp, "Levl %d\n",	ch->level		);
    if (ch->trust != 0)
    ...
    fprintf( fp, "Impp  %ld\n",   ch->pcdata->imppt         );
	fprintf( fp, "Slay %d\n", ch->pcdata->slay_cnt);
	fprintf( fp, "Expbonus %d\n", ch->pcdata->exp_bonus);/* 031601 */
	fprintf( fp, "Exptimer %d\n", ch->pcdata->exp_timer);/* 031601 */

	fprintf( fp, "Deity %d\n", ch->pcdata->deity);/* 042401 */
	...
	/* 033102 */
	fprintf( fp, "QstF "); /*033102*/
	for(Loop = 0; Loop <= MAX_QUEST; Loop++)
		fprintf( fp, "%c", ch->pcdata->quest_flags[Loop]);
	fprintf( fp, "~\n");/*033102*/
    fprintf( fp, "Comm %s\n",       print_flags(ch->comm));
    if (ch->wiznet)
	...
	fprintf( fp, "Titl %s~\n",	ch->pcdata->title	);
    	fprintf( fp, "Pnts %d\n",   	ch->pcdata->points      );
	fprintf( fp, "TSex %d\n",	ch->pcdata->true_sex	);
/* added TAKA banking system */
	fprintf( fp, "Balance     %ld\n",        ch->pcdata->balance );
	fprintf( fp, "Balance2    %ld\n",        ch->pcdata->sbalance );
	fprintf( fp, "DLimit      %d\n",        ch->pcdata->dailylimit );
	fprintf( fp, "NoShare	  %d\n",		NO_SHARES);

	/* birthday write to pfile fix by taka */
	fprintf( fp, "Syer        %d\n",		ch->startyear);
	fprintf( fp, "Smnt        %d\n",        ch->startmonth);
	fprintf( fp, "Sday        %d\n",		ch->startday);
	/*060402 by Taka*/
	...
	{
		fprintf( fp, " %d",ch->pcdata->shares[sIndex]);
	}
	fprintf( fp, "\n");
/* end TAKA Banking system */
/* store host in pc file */
	if(ch->desc != NULL)
		fprintf( fp, "HostN	%s~\n", ch->desc->host);
/* mariage code by TAKA */
	fprintf( fp, "Spouse %s~\n", ch->pcdata->spouse);
/* TAKA 0314 */
	fprintf( fp, "Pname %s~\n", ch->pcdata->personal_name);
	fprintf( fp, "Pnick %s~\n", ch->pcdata->personal_nickname);
	fprintf( fp, "Pnote %s~\n", ch->pcdata->personal_note);
	fprintf( fp, "Pemai %s~\n", ch->pcdata->personal_email);
	fprintf( fp, "Pweba %s~\n", ch->pcdata->personal_webaddress);
	fprintf( fp, "Picqn %s~\n", ch->pcdata->personal_icqno);

	fprintf( fp, "SPW %s~\n", ch->pcdata->stor_pw);/* 070201 */
	fprintf( fp, "Pmode %d\n", ch->pcdata->play_mode);/*070201b*/
...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name )
{
    ...
    ch->pcdata->rstmsg			= str_dup( "" ); /* 1129A Taka rsetore message */
    ch->pcdata->whotitl			= str_dup( "" ); /* 1214  Taka who title */
    ch->pcdata->spouse			= str_dup( "" );
    ch->pcdata->stor_pw			= str_dup( "" );/* 070201 */
	ch->pcdata->play_mode		= 0;			/*070201b*/
    ...
    ch->pcdata->hrecall = ROOM_VNUM_TEMPLE;
	...
	/* paintball war initialize */
	ch->pcdata->char_in_pb_war = FALSE;
	ch->pcdata->combatant_number = 0;
	ch->pcdata->number_shoots = 0;
	ch->pcdata->noshares = 0;
	for(sIndex = 0; sIndex <= NO_SHARES; sIndex++)
		ch->pcdata->shares[sIndex] = 0;
    /* TAKA 0314 */
	ch->pcdata->personal_name		= str_dup( "" );
    ch->pcdata->personal_nickname	= str_dup( "" );
    ch->pcdata->personal_email		= str_dup( "" );
    ch->pcdata->personal_webaddress	= str_dup( "" );
    ch->pcdata->personal_icqno		= str_dup( "" );
    ch->pcdata->personal_note		= str_dup( "" );
	ch->pcdata->rebirth = FALSE; /* 041801 -initialize to flase on player load */
	ch->pcdata->deity = 0;
	ch->pcdata->bank_wait = 0;/*060402 by Taka*/
	for(Loop = 0; Loop < MAX_QUEST; Loop++)
		ch->pcdata->quest_flags[Loop] = '0';
	...
	for (i = 0; i < 5; i++)
	{
	    if (ch->ri->skills[i] == NULL)
		break;
	    group_add(ch,ch->ri->skills[i],FALSE);
	}
	ch->affected_by = ch->affected_by|ch->ri->aff;
	ch->imm_flags	= ch->imm_flags | ch->ri->imm;
	ch->res_flags	= ch->res_flags | ch->ri->res;
	ch->vuln_flags	= ch->vuln_flags | ch->ri->vuln;
	ch->form	= ch->ri->form;
	ch->parts	= ch->ri->parts;
    }
...
void fread_char( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'B':
		...
		KEY( "Balance2", ch->pcdata->sbalance, fread_number( fp ) );
		KEY( "Bwait", ch->pcdata->bank_wait, fread_number( fp ) ); /*060402 by Taka*/
	    KEYS( "Bamfin",	ch->pcdata->bamfin,	fread_string( fp ) );			/*070201a*/
	    KEYS( "Bamfout",	ch->pcdata->bamfout,	fread_string( fp ) );	/*070201a*/
	    KEYS( "Bin",		ch->pcdata->bamfin,	fread_string( fp ) );		/*070201a*/
	    KEYS( "Bout",	ch->pcdata->bamfout,	fread_string( fp ) );		/*070201a*/
	    break;

	case 'C':
	    KEY( "Class",	ch->class,		fread_number( fp ) );
	    KEY( "Cla",		ch->class,		fread_number( fp ) );
/*	    KEY( "Clan",	ch->clan,	clan_lookup(fread_string(fp))); 1228 */
/*1228*/KEY( "ClRk",	ch->clan_rank,	fread_number( fp ) );
/*1228*/KEY( "Clrk",	ch->clan_rank,	fread_number( fp ) );
		...
		if ( !str_cmp( word, "Clan" ) )
	    {
			...
			for(cid=CLhead; cid != NULL; cid=cid->CLnext)
			{
				if (LOWER(word[0]) == LOWER(cid->short_name[0])
				&&  !str_prefix( word,cid->short_name))
				{
					ch->cid = cid;
					ch->clan = cid->clan_number;
					ch->clan_rank = 0;
					fMatch = TRUE;
					break;
				}
	...
	case 'D':
	    KEY( "DLimit",	ch->pcdata->dailylimit,		fread_number( fp ) );
	    KEY( "Damroll",	ch->damroll,		fread_number( fp ) );
	    KEY( "Dam",		ch->damroll,		fread_number( fp ) );
	    KEY( "Deity",	ch->pcdata->deity,	fread_number( fp ) );
	    KEYS( "Description",	ch->description,	fread_string( fp ) );	/*070201a*/
	    KEYS( "Desc",	ch->description,	fread_string( fp ) );		/*070201a*/
	...
	case 'E':
	    ...
	    KEY( "Expbonus",	ch->pcdata->exp_bonus,		fread_number( fp ) );/* 031601 */
	    KEY( "Exptimer",	ch->pcdata->exp_timer,		fread_number( fp ) );/* 031601 */
	    break;
	...
	case 'H':
	    ...
	    KEYS( "Hist",	ch->pcdata->history, fread_string( fp ) );/*070201a*/
	    KEYS( "HostN",	Host, fread_string( fp ) );

	    if ( !str_cmp( word, "HRec" ) )
	    {
		ch->pcdata->hrecall = fread_long_number( fp );
		fMatch = TRUE;
		break;
	    }
	...
	case 'I':
        ...
        KEY( "Immp",    ch->pcdata->immpt,    fread_long_number( fp ) );
        KEY( "Impp",    ch->pcdata->imppt,    fread_long_number( fp ) );
	    break;
	...
	case 'L':
	    ...
	    KEYS( "LongDescr",	ch->long_descr,		fread_string( fp ) );/*070201a*/
	    KEYS( "LnD",		ch->long_descr,		fread_string( fp ) );/*070201a*/
	    break;

	case 'N':
	    KEYS( "Name",	ch->name,		fread_string( fp ) );
	    KEY( "Note",	ch->pcdata->last_note,	fread_number( fp ) );
	    KEY( "NoShare",	ch->pcdata->noshares,	fread_number( fp ) );
	    if (!str_cmp(word,"Not"))
	    {
	...
	case 'P':
	    KEYS( "Password",	ch->pcdata->pwd,	fread_string( fp ) );/*070201a*/
	    KEYS( "Pass",	ch->pcdata->pwd,	fread_string( fp ) );	/*070201a*/
...
/* 	    KEY( "Prom",	ch->prompt,		fread_string( fp ) ); */
		...
		/* TAKA 0314 */
		KEYS( "Pname",	ch->pcdata->personal_name,			fread_string( fp ) );
		KEYS( "Pnick",	ch->pcdata->personal_nickname,		fread_string( fp ) );
		KEYS( "Pnote",	ch->pcdata->personal_note,			fread_string( fp ) );
		KEYS( "Pemai",	ch->pcdata->personal_email,			fread_string( fp ) );
		KEYS( "Pweba",	ch->pcdata->personal_webaddress,	fread_string( fp ) );
		KEYS( "Picqn",	ch->pcdata->personal_icqno,			fread_string( fp ) );
		/* Play Mode 070201b */
		KEY( "Pmode",	ch->pcdata->play_mode,				fread_number( fp ) );/*070201b*/
		break;

	case 'Q':/*033102*/
		if ( !str_cmp( word, "QstF" ) )
		{
			qFlag = fread_string( fp );
			for(Loop = 0; Loop <
				MAX_QUEST; Loop++)
				ch->pcdata->quest_flags[Loop] = qFlag[Loop];
		}
		break;

	case 'R':
/*070201a*/ KEYS( "RstM",	ch->pcdata->rstmsg,	fread_string( fp ) ); /* 1129A Taka Custom Restore Message */
	    if ( !str_cmp( word, "Race" ) )
	    {
			...
			{
				if (LOWER(word[0]) == LOWER(ri->name[0])
				&&  !str_prefix( word,ri->name))
				{
					ch->ri = ri;
	    ...
	    {
		ch->in_room = get_room_index( fread_long_number( fp ) );
		if ( ch->in_room == NULL )
		    ch->in_room = get_room_index( ROOM_VNUM_LIMBO );
		fMatch = TRUE;
		break;
	    }
	    if (!str_cmp(word,"RNot"))
	    {
		ch->pcdata->last_rnote			= fread_number(fp);
		fMatch = TRUE;
		break;
	    }
	    break;
	...
	case 'S':
	    ...
	    KEYS( "ShortDescr",	ch->short_descr,	fread_string( fp ) );/*070201a*/
	    KEYS( "ShD",		ch->short_descr,	fread_string( fp ) );/*070201a*/
	    KEY( "Sec",         ch->pcdata->security,	fread_number( fp ) );	/* OLC */
		KEYS( "SPW",		ch->pcdata->stor_pw,	fread_string( fp ) ); /* 070201 */
	   	/*
		This code replaced by TAKA     to add mud time to a file for better
		tracking :) Also for our own birthday code logic!
		*/
		KEY( "Syer",	ch->startyear, 		fread_number( fp ) );
	    KEY( "Smnt",	ch->startmonth,		fread_number( fp ) );
	    KEY( "Sday",	ch->startday,		fread_number( fp ) );
	    KEYS( "Spouse",	ch->pcdata->spouse,		fread_string( fp ) );
	    /* TAKA     end time */

        KEY( "Silv",        ch->silver,             fread_number( fp ) );
        KEY( "Slay",        ch->pcdata->slay_cnt,   fread_number( fp ) );

		/* TAKA banking system adds */
		if ( !str_cmp( word, "Shares"))
		{
			if(ch->pcdata->noshares == 0)
				ch->pcdata->noshares = NO_SHARES;

 			for(sIndex = 1; sIndex <= ch->pcdata->noshares; sIndex++)
			{
				if(ch->pcdata->noshares <= NO_SHARES)
					ch->pcdata->shares[sIndex] =  fread_number( fp );
			}
			fMatch = TRUE;
			break;
		}
		/* end adds TAKA banking system */
	...
	case 'W':
	    ...
	    KEYS( "WhoT",	ch->pcdata->whotitl,		fread_string( fp ) ); /* 1214 */
	    break;
...
void fread_obj( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'V':
	    ...
	    {
		obj->value[0]	= fread_long_number( fp );
		obj->value[1]	= fread_long_number( fp );
		obj->value[2]	= fread_long_number( fp );
		obj->value[3]	= fread_long_number( fp );
		...
		obj->value[0] 	= fread_long_number( fp );
	 	...
	 	obj->value[2] 	= fread_long_number( fp );
		obj->value[3]	= fread_long_number( fp );
		obj->value[4]	= fread_long_number( fp );
		fMatch = TRUE;
		break;
	    }
