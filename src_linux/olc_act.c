...
void show_obj_values( CHAR_DATA *ch, OBJ_INDEX_DATA *obj )
{
    ...
    case ITEM_WINDOW:   /* 1201A */
	    sprintf( buf, "[v0] Room to view: [%ld]\n\r", obj->value[0] );
		send_to_char( buf, ch );
		break;
...
bool set_obj_values( CHAR_DATA *ch, OBJ_INDEX_DATA *pObj, int value_num, char *argument)
{
    switch( pObj->item_type )
    {
    	...
    	case ITEM_WINDOW: /* 1201A for use with OLC */
	    switch ( value_num )
	    {
	        default:
		    	send_to_char( "set v0 to the window, or to zero if you wish to use the windows description \n\r",ch);
	            return FALSE;
	        case 0:
	            send_to_char( "Window Set.\n\r\n\r", ch );
...
OEDIT( oedit_show )
{
	...
	/*0902*/
  /*  sprintf( buf, "Object codes:    [%s]\n\r", pObj->uoc.ShowCodes( ) );
    send_to_char( buf, ch );*/
...
MEDIT( medit_show )
{
    ...
    sprintf( buf, "Hit dice:    [%5ldd%-5ld+%8ld] ",
	     pMob->hit[DICE_NUMBER],
	     pMob->hit[DICE_TYPE],
	     pMob->hit[DICE_BONUS] );
    send_to_char( buf, ch );

    sprintf( buf, "Damage dice: [%5ldd%-5ld+%8ld] ",
	     pMob->damage[DICE_NUMBER],
	     pMob->damage[DICE_TYPE],
	     pMob->damage[DICE_BONUS] );
    send_to_char( buf, ch );

    sprintf( buf, "Mana dice:   [%5ldd%-5ld+%8ld]\n\r",
...
MEDIT( medit_race )
{
	...
	pMob->race = race;
	pMob->act	  |= ri->act;
	pMob->affected_by |= ri->aff;
	pMob->off_flags   |= ri->off;
	pMob->imm_flags   |= ri->imm;
	pMob->res_flags   |= ri->res;
	pMob->vuln_flags  |= ri->vuln;
	pMob->form        |= ri->form;
	pMob->parts       |= ri->parts;
...
void show_liqlist(CHAR_DATA *ch)
{
	...
	sprintf(buf, "%-20s %-14s %5ld %4ld %6ld %4ld %5ld\n\r",
...
MEDIT ( medit_addmprog )
{
  ...
  if ( ( code =get_prog_index (atol(num), 0 ) ) == NULL)
  {
...
/*OEDIT( oedit_uoc )
{
  ...
  return pObj->uoc.SelectCodes( ch, argument );
}
*/
...
OEDIT( oedit_delete )
{
     ...
     if ( argument[0] == '\0' )
     {
          send_to_char( "{wSyntax:  {Codelete {c[{wvnum{c]{x\n\r", ch );
          return FALSE;
     }
     ...
     sprintf( buf, "{wRemoved object vnum {C%d{w and {C%d{w resets.{x\n\r", index, count );
     send_to_char( buf, ch );
     return TRUE;
}
...
MEDIT( medit_delete )
{
     ...
     if ( argument[0] == '\0' )
     {
          send_to_char( "{wSyntax:  {Cmdelete {c[{wvnum{c]{x\n\r", ch );
          return FALSE;
     }
     ...
     {
          send_to_char( "{wThat is not a number.{x\n\r", ch );
          return FALSE;
     }

     if( !pMob )
     {
          send_to_char( "{wNo such mobile.{x\n\r", ch );
          return FALSE;
     }
...
*/
     ...
     /* The reason for the kludge is that sometime after the exit of medit_delete */
     /* ch->mount becomes 0x10000000 and I have no idea how or where */
     /*   Narbo */
/*     if( ch->mount == NULL )
          medit_delete_kludge = ch; */

     sprintf( buf, "{wRemoved mobile vnum {C%d{w and {C%d{w resets.{x\n\r", index, count );
     send_to_char( buf, ch );
     return TRUE;
}
...
OEDIT( oedit_autoarmor )
{
	...
	size = UMAX(1, pObj->level/(28 / 10) + 1);
	pObj->weight = pObj->level + 1;
...
MEDIT( medit_autoset )
{
	...
	ac_n = 95 - (pMob->level * (667 / 100)) - ((pMob->level/10)^2);
	ac_x = 95 - (pMob->level * (457 / 100)) - ((pMob->level/10)^2);
	...
	size = ((87 / 100) + pMob->level/dice);

	if (size < 2)
		size = 2;

	bonus = ((55 / 10) + pMob->level/2);
	sprintf(temp, "%dd%d + %d", dice, size, bonus);
...
MEDIT( medit_autohard )
{
	...
	ac_n = 88 - (pMob->level * (705 / 100)) - ((pMob->level/10)^2);
	ac_x = 88 - (pMob->level * (502 / 100)) - ((pMob->level/10)^2);
	...
	bonus = ((750 / 100) + pMob->level/(150 / 100));
	sprintf(temp, "%dd%d + %d", dice, size, bonus);
	medit_damdice(ch, temp);
	bonus = (pMob->level/53 + 1) * ((pMob->level * 10) + (pMob->level/10)) +1; 
	size = pMob->level/3;
	...
	size++;
	dice = (pMob->level + (6 / 10)) * (205 / 100) + (5 / 10);
...
MEDIT( medit_autoeasy )
{
	...
	ac_n = 99 - (pMob->level * (637 / 100)) - ((pMob->level/10)^2);
	ac_x = 99 - (pMob->level * (427 / 100)) - ((pMob->level/10)^2);
	/* sprintf(temp, "%d %d %d %d", ac_n, ac_n, ac_n, ac_x);*/
	pMob->ac[AC_PIERCE] = ac_n;
	pMob->ac[AC_BASH]   = ac_n;
	pMob->ac[AC_SLASH]  = ac_n;
	...
	sprintf(temp, "%dd%d + %d", dice, size, bonus);
	medit_manadice(ch, temp);
	dice = pMob->level/3 * (95 / 100);

	if (dice < 1)
		dice = 1;

	size = ((87 / 100) + pMob->level/dice) * (95 / 100);
	
	if (size < 2)
		size = 2;

	bonus = ((250 / 100) + pMob->level/(210 / 100));
	sprintf(temp, "%dd%d + %d", dice, size, bonus);
	medit_damdice(ch, temp);
