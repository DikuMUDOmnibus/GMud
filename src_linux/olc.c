...
/*    {	"Object Codes",	oedit_uoc	}, *//*0902*/
	{ "copy", oedit_copy },
	{	"delete",	oedit_delete	},  /* <----RIGHT HERE */
...
/*  {   command		function	}, */
	...
	{ "copy", medit_copy },
	{	"delete",	medit_delete	},  /* <----RIGHT HERE */
	{	"autoset",	medit_autoset	},  /* <----RIGHT HERE */
	{	"autoeasy",	medit_autoeasy	},  /* <----RIGHT HERE */
	{	"autohard",	medit_autohard	},  /* <----RIGHT HERE */
...
void aedit( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "{CAEdit:  {cInsufficient security to modify area.{x\n\r", ch );
	edit_done( ch );
	return;
    }
...
void redit( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_BUILDER( ch, pArea ) )
    {
        send_to_char( "{CREdit:  {cInsufficient security to modify room.{x\n\r", ch );
	edit_done( ch );
	return;
    }
...
void oedit( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_BUILDER( ch, pArea ) )
    {
	send_to_char( "{COEdit: {cInsufficient security to modify area.{x\n\r", ch );
	edit_done( ch );
	return;
    }
...
void medit( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_BUILDER( ch, pArea ) )
    {
	send_to_char( "{CMEdit: {cInsufficient security to modify area.{x\n\r", ch );
	edit_done( ch );
	return;
    }
...
void do_aedit( CHAR_DATA *ch, char *argument )
{
	...
	{
	    send_to_char( "{cThat area vnum does not exist.{x\n\r", ch );
	    return;
	}
    }
    else
	...
	{
		send_to_char( "{CAEdit : {cInsuffecient security to create new area.{c\n\r", ch );
		return;
	}
    ...
    if (!IS_BUILDER(ch,pArea))
    {
	send_to_char("{cInsuffecient security to edit areas.{x\n\r",ch);
	return;
    }
...
void do_redit( CHAR_DATA *ch, char *argument )
{
	...
	{
		send_to_char( "{cInsuffecient security to modify rooms.{x\n\r" , ch );
        	return;
	}
	...
	if ( !pRoom )
	{
		send_to_char( "{CREdit : {cRoom does not exist.{x\n\r", ch );
		return;
	}

	if ( !IS_BUILDER(ch, pRoom->area) )
	{
		send_to_char( "{cREdit : {cInsuffecient security to edit room.{x\n\r", ch );
		return;
	}
...
void do_oedit( CHAR_DATA *ch, char *argument )
{
	...
	{
	    send_to_char( "{COEdit:  {cThat vnum does not exist.{x\n\r", ch );
	    return;
	}

	if ( !IS_BUILDER( ch, pObj->area ) )
	{
		send_to_char( "Insuffecient security to modify objects.{x\n\r" , ch );
	        return;
	}
	...
	if ( !str_cmp( arg1, "create" ) )
	{
	    ...
	    {
		send_to_char( "{CSyntax:  {cedit object create {g[{cvnum{g]{x\n\r", ch );
		return;
	    }
	    ...
	    if ( !pArea )
	    {
		send_to_char( "{COEdit:  {cThat vnum is not assigned an area.{x\n\r", ch );
		return;
	    }

	    if ( !IS_BUILDER( ch, pArea ) )
	    {
		send_to_char( "{cInsuffecient security to modify objects.{x\n\r" , ch );
	        return;
	    }
    ...
    send_to_char( "{COEdit:  {cThere is no default object to edit.{x\n\r", ch );
    return;
}
...
void do_medit( CHAR_DATA *ch, char *argument )
{
	...
	{
	    send_to_char( "{CMEdit:  {cThat vnum does not exist.{x\n\r", ch );
	    return;
	}

	if ( !IS_BUILDER( ch, pMob->area ) )
	{
		send_to_char( "{cInsuffecient security to modify mobs.{x\n\r" , ch );
	        return;
	}
	...
	if ( !str_cmp( arg1, "create" ) )
	{
	    ...
	    if ( arg1[0] == '\0' || value == 0 )
	    {
		send_to_char( "{CSyntax:  {cedit mobile create {g[{cvnum{g]{x\n\r", ch );
		return;
	    }
    ...
    send_to_char( "{CMEdit:  {cThere is no default mobile to edit.{x\n\r", ch );
    return;
}
...
void display_resets( CHAR_DATA *ch )
{
    ...
    send_to_char (
  "{c No.  Loads    Description       Location         Vnum   Mx Mn Description"
  "{x\n\r"
  "{C==== ======== ============= =================== ======== ===== ==========="
  "{x\n\r", ch );
	...
	{
	default:
	    sprintf( buf, "{cBad reset command: {C%c.{x\n\r", pReset->command );
	    strcat( final, buf );
	    break;
	...
	case 'M':
	    ...
	    {
                sprintf( buf, "{cLoad Mobile - Bad Mob {C%ld{x\n\r", pReset->arg1 );
                strcat( final, buf );
                continue;
	    }
	    ...
	    {
                sprintf( buf, "{cLoad Mobile - Bad Room {C%ld{x\n\r", pReset->arg3 );
                strcat( final, buf );
                continue;
	    }
	...
	case 'O':
	    ...
	    {
                sprintf( buf, "{cLoad Object - Bad Object {C%ld{x\n\r",
		    pReset->arg1 );
                strcat( final, buf );
                continue;
	    }
	    ...
	    if ( !( pRoomIndex = get_room_index( pReset->arg3 ) ) )
	    {
                sprintf( buf, "{cLoad Object - Bad Room {C%ld{x\n\r", pReset->arg3 );
                strcat( final, buf );
                continue;
	    }
	...
	case 'P':
	    if ( !( pObjIndex = get_obj_index( pReset->arg1 ) ) )
	    {
                sprintf( buf, "{cPut Object - Bad Object {C%ld{x\n\r",
                    pReset->arg1 );
                strcat( final, buf );
                continue;
	    }
	    ...
	    {
                sprintf( buf, "{cPut Object - Bad To Object {C%ld{x\n\r",
                    pReset->arg3 );
                strcat( final, buf );
                continue;
	    }
	...
	case 'E':
	    if ( !( pObjIndex = get_obj_index( pReset->arg1 ) ) )
	    {
                sprintf( buf, "{cGive/Equip Object - Bad Object {C%ld{x\n\r",
                    pReset->arg1 );
                strcat( final, buf );
                continue;
	    }
	    ...
	    if ( !pMob )
	    {
                sprintf( buf, "{cGive/Equip Object - No Previous Mobile{x\n\r" );
                strcat( final, buf );
                break;
	    }
	...
	case 'R':
	    ...
	    {
		sprintf( buf, "{cRandomize Exits - Bad Room {C%ld{x\n\r",
		    pReset->arg1 );
		strcat( final, buf );
		continue;
	    }

	    sprintf( buf, "{WR{g[{c%5ld{g] {wExits are randomized in {C%s{x\n\r",
...
void do_resets( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "{CResets: {cInvalid security for editing this area.{x\n\r",
                      ch );
	return;
    }
	...
	{
	    send_to_char(
		"{cResets: M = mobile, R = room, O = object, {x"
		"{cP = pet, S = shopkeeper{x\n\r", ch );
	    display_resets( ch );
	}
	else
	    send_to_char( "{cNo resets in this room.{x\n\r", ch );
    }
	...
	if ( !str_cmp( arg2, "delete" ) )
	{
	    ...
	    if ( !ch->in_room->reset_first )
	    {
		send_to_char( "{cNo resets in this area.{x\n\r", ch );
		return;
	    }
		...
		if ( !pReset )
		{
		    send_to_char( "{cReset not found.{x\n\r", ch );
		    return;
		}
	  ...
	  || (!str_cmp( arg2, "obj" ) && is_number( arg3 )) )
	{
		...
		if (get_mob_index( is_number(arg3) ? atoi( arg3 ) : 1 ) == NULL)
		  {
		    send_to_char("{cMob does not exist.{x\n\r",ch);
		    return;
		  }
		...
		if ( !str_prefix( arg4, "inside" ) )
		{
		    ...
		    if ( ( temp->item_type != ITEM_CONTAINER ) &&
		     ...
		     {
		       send_to_char( "{cObject to is not a container.{x\n\r", ch);
		       return;
		     }
		    pReset->command = 'P';
		...
		if ( !str_cmp( arg4, "room" ) )
		{
		    if (get_obj_index(atoi(arg3)) == NULL)
		      {
		         send_to_char( "{cVnum does not exist.{x\n\r",ch);
		         return;
		      }
		    pReset->command  = 'O';
		    pReset->arg2     = 0;
		 ...
		 */
		{
		    ...
		    {
			send_to_char( "{CResets: {c'? wear-loc'{x\n\r", ch );
			return;
		    }
		    if (get_obj_index(atoi(arg3)) == NULL)
		      {
	...
	if (!str_cmp( arg2, "random") && is_number(arg3))
	{
		if (atoi(arg3) < 1 || atoi(arg3) > 6)
			{
				send_to_char("{cInvalid argument.{x\n\r", ch);
				return;
			}
		pReset = new_reset_data ();
		pReset->command = 'R';
	...
	{
	send_to_char( "{CSyntax: {cRESET {g<{cnumber{g> {cOBJ {g<{cvnum{g> {g<{cwear_loc{g>{x\n\r", ch );
	send_to_char( "        {cRESET {g<{cnumber{g> {cOBJ {g<{cvnum{g> {cinside {g<{cvnum{g> [{climit{g] [{ccount{g]{x\n\r", ch );
	send_to_char( "        {cRESET {g<{cnumber{g> {cOBJ {g<{cvnum{g> {croom{x\n\r", ch );
	send_to_char( "        {cRESET {g<{cnumber{g> {cMOB {g<{cvnum{g> {g[{cmax #x area{g] [{cmax #x room{g]{x\n\r", ch );
	send_to_char( "        {cRESET {g<{cnumber{g> {cDELETE{x\n\r", ch );
	send_to_char( "        {cRESET {g<{cnumber{g> {cRANDOM {g[{c#x exits{g]{x\n\r", ch);
	}
    }
...
void do_alist( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( result, "{g[{c%3s{g] [{c%-27s{g] ({c%-5s-%5s{g) [{c%-10s{g] {c%3s {g[{c%-10s{g]{x\n\r",
       "Num", "Area Name", "lvnum", "uvnum", "Filename", "Sec", "Builders" );

    for ( pArea = area_first; pArea; pArea = pArea->next )
    {
	sprintf( buf, "{g[{c%3ld{g] {c%-29.29s {g({c%-5ld-%5ld{g) {c%-12.12s {g[{c%d{g] [{c%-10.10s{g]{x\n\r",
	     pArea->vnum,
	     pArea->name,
	     pArea->min_vnum,
	     pArea->max_vnum,
