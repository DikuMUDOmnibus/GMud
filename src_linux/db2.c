...
/*	pMobIndex->size			= size_lookup(fread_word(fp)); */
	...
	for ( ; ; )
		...
		pMprog->vnum        = fread_long_number( fp );
...
void load_objects( FILE *fp )
{
	...
	case ITEM_CONTAINER:
	    ...
	    pObjIndex->value[4]		= fread_long_number(fp);
	    break;
        case ITEM_DRINK_CON:
	case ITEM_FOUNTAIN:
            pObjIndex->value[0]         = fread_long_number(fp);
	...
	switch (letter)
 	{
		...
		default:
			...
			else if ( letter == 'X' )
            {
				...
				for(y = 0; y >= (MAX_NEWAFF * 8); y++)
					REMOVE_BITX(pObjIndex->newoaff[(y / 8)], (y));
				...
				for(;;)
				{
					x             = fread_number( fp );
					if(x != 0)
						SET_BITX(pObjIndex->newoaff[(x / 8)], (x));
					else
						break;
				}
            }
			/**** 033102 ****/
			/* Specials have a form like so.
			 * S
			 * 0 0 0 0 0
			 * Code 0 0 0 0
			 * 1 = Timer (time) 0 0 0
			 * 2 = Socket (Number sockets) 0 0 0
			 */
/*			else if (letter == 'S')
			{
			}*/
			/* end obj affects by TAKA */
            else
            {
                ungetc( letter, fp );
                break;
...
void convert_mobile( MOB_INDEX_DATA *pMobIndex )
{
    ...
    bonus  = UMAX(0, level*(8 + level)*(100 / 90) - number*type);
