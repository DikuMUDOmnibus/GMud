...
void do_cast (CHAR_DATA * ch, char *argument)
{
	...
	{
			act("You are blind.",ch,NULL,NULL,TO_CHAR);
			return;
	}
	/* 1212 */
						   ...
						   int target)
{
	...
	printf_to_char (ch, "%s's lightning strikes your foes!\n\r", 
		deity_table[ch->pcdata->deity].name);
	sprintf(buf, "$n calls %s's lightning to strike $s foes!",
		deity_table[ch->pcdata->deity].name);
...
void spell_calm (int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	if (number_range (0, chance) >= mlevel) {	/* hard to stop large fights */
		for (vch = ch->in_room->people; vch != NULL; vch = vch->next_in_room) {
			...
			{
				send_to_char("Calm does not work on the undead!", ch);
				return;
			}
						 ...
						 int target)
{
	...
	{
		found = TRUE;
		act("$n is no longer forced to sleek.",victim,NULL,NULL,TO_ROOM);
		/*wake( victim, "");*/
	}
...
/* TAKA     SPELLS HERE */
	if (check_dispel(level,victim,skill_lookup("stone meld")))
    {
        act("$n no longer made of stone.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }
    if (check_dispel(level,victim,skill_lookup("screen")))
    {
        act("$n no longer enveloped by smoke.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }
    if (check_dispel(level,victim,skill_lookup("adrenaline")))
    {
        act("$n no longer pumped up.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }

    if (check_dispel(level,victim,skill_lookup("mud skin")))
    {
        act("$n's skin becomes clean.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }

	if (check_dispel(level,victim,skill_lookup("moss skin")))
    {
        act("$n's skin losses it's plant like texture.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }

    if (check_dispel(level,victim,skill_lookup("bark skin")))
    {
        act("$n's skin is no longer protected by bark.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }

    if (check_dispel(level,victim,skill_lookup("steel skin")))
    {
        act("$n's skin is no longer encased in steel.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }

    if (check_dispel(level,victim,skill_lookup("emerald skin")))
    {
        act("$n's skin loses it's greenish color.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }

    if (check_dispel(level,victim,skill_lookup("ruby skin")))
    {
        act("$n's skin loses it's redish color.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }

    if (check_dispel(level,victim,skill_lookup("diamond skin")))
    {
        act("$n's skin loses it's lustor.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }

    if (check_dispel(level,victim,skill_lookup("hit")))
    {
        act("$n no longer seems less coordinated.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }

    if (check_dispel(level,victim,skill_lookup("damage")))
    {
        act("$n no longer hits as hard as $e used to.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }

    if (check_dispel(level,victim,skill_lookup("mystic armor")))
    {
        act("$n no longer has a mystical glow.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }

    if (check_dispel(level,victim,skill_lookup("fighting trance")))
    {
        act("$n is no longer as ready for battle.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }

    if (check_dispel(level,victim,skill_lookup("bloody tears")))
    {
        act("$n no longer bleeds from $s eyes.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }


/* End TAKA spells here */
    ...
    if (check_dispel (level, victim, skill_lookup ("vapour"))) {
		act ("$n looks stronger.", victim, NULL, NULL, TO_ROOM);
		found = TRUE;
	}

    if (check_dispel (level, victim, skill_lookup ("smog"))) {
		act ("$n looks stronger.", victim, NULL, NULL, TO_ROOM);
		found = TRUE;
	}
    ...
    if (check_dispel (level, victim, skill_lookup ("camouflage"))) {
		act ("$n looks stronger.", victim, NULL, NULL, TO_ROOM);
		found = TRUE;
	}

    if (check_dispel (level, victim, skill_lookup ("faith"))) {
		act ("$n looks stronger.", victim, NULL, NULL, TO_ROOM);
		found = TRUE;
	}

    if (check_dispel (level, victim, skill_lookup ("vengence"))) {
		act ("$n looks stronger.", victim, NULL, NULL, TO_ROOM);
		found = TRUE;
	}

    if (check_dispel (level, victim, skill_lookup ("brain bulge"))) {
		act ("$n looks stronger.", victim, NULL, NULL, TO_ROOM);
		found = TRUE;
	}

    if (check_dispel (level, victim, skill_lookup ("vitality"))) {
		act ("$n looks stronger.", victim, NULL, NULL, TO_ROOM);
		found = TRUE;
	}

    if (check_dispel (level, victim, skill_lookup ("enlightenment"))) {
		act ("$n looks stronger.", victim, NULL, NULL, TO_ROOM);
		found = TRUE;
	}
	...
	if (found)
		send_to_char ("Cancelation completed.\n\r", ch);
	else
...
void spell_curse (int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	if (IS_AFFECTED (victim, AFF_CURSE)
		|| saves_spell (level, victim, DAM_NEGATIVE)) return;
	af.where = TO_AFFECTS;
	af.type = sn;
	af.level = level;
	af.duration = 2 * level;
						...
						int target)
{
	...
	if (IS_GOOD (victim)) 
	{
		sprintf(buf, "%s protects $N.", deity_table[ch->pcdata->deity].name);
						 ...
						 int target)
{
    ...
    if (check_dispel (level, victim, skill_lookup ("vapour"))) {
		...
		act ("The vapour around $n evaporates.", victim, NULL, NULL, TO_ROOM);
    ...
    if (check_dispel (level, victim, skill_lookup ("smog"))) {
		...
		act ("The smog surrounding $n disappears.", victim, NULL, NULL, TO_ROOM);
    ...
    if (check_dispel (level, victim, skill_lookup ("smoke screen"))) {
		...
		act ("The smoke clears and you can see $n.", victim, NULL, NULL, TO_ROOM);
    ...
    if (check_dispel (level, victim, skill_lookup ("camouflage"))) {
		...
		act ("$n wipes the camouflage paint off of themself.", victim, NULL, NULL, TO_ROOM);
    ...
    if (check_dispel (level, victim, skill_lookup ("faith"))) {
		...
		act ("$n has lost their faith.", victim, NULL, NULL, TO_ROOM);
    ...
    if (check_dispel (level, victim, skill_lookup ("vengence"))) {
		...
		act ("$n is no longer enraged.", victim, NULL, NULL, TO_ROOM);
    ...
    if (check_dispel (level, victim, skill_lookup ("brain bulge"))) {
		...
		act ("$n is moronic!", victim, NULL, NULL, TO_ROOM);
    ...
    if (check_dispel (level, victim, skill_lookup ("vitality"))) {
		...
		act ("$n's vitality has deminished.", victim, NULL, NULL, TO_ROOM);
    ...
    if (check_dispel (level, victim, skill_lookup ("enlightenment"))) {
		...
		act ("$n is no longer wise.", victim, NULL, NULL, TO_ROOM);
    ...
    if (check_dispel (level, victim, skill_lookup ("confusion"))) {
		...
		act ("$n no longer has a confused look on their face.", victim, NULL, NULL, TO_ROOM);
...
void spell_enchant_armor( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    /* the moment of truth */
    if ((result < (fail / 5)) && (ch->level <= (MAX_LEVEL - 1)))  /* item destroyed */
    {
		if (ch->level >= (MAX_LEVEL - 5))
		{
			act("$p item has been saved!",ch,obj,NULL,TO_CHAR);
			act("$p flares blindingly... but is saved!",ch,obj,NULL,TO_ROOM);
		}
		else
		{
    ...
    if ((result < (fail / 3)) && (ch->level <= (MAX_LEVEL - 1))) /* item disenchanted */
    {
    ...
    if (( result <= fail ) && (ch->level <= (MAX_LEVEL - 1)))  /* failed, no bad result */
    {
...
void spell_enchant_weapon(int sn,int level,CHAR_DATA *ch, void *vo,int target)
{
    ...
    if ((result < (fail / 2)) && (ch->level <= (MAX_LEVEL - 1))) /* item disenchanted */
    {
	AFFECT_DATA *paf_next;
...
void spell_lightning_strike (int sn, int level, CHAR_DATA * ch, void *vo,
						   int target)
{
	CHAR_DATA *victim = (CHAR_DATA *) vo;
	static const sh_int dam_each[] = {
						  ...
						  int target)
{
	...
	sprintf (buf1, "{W%s {Gsays '%s'{x\n\r", speaker, target_name);
	sprintf (buf2, "{WSomeone {Gmakes {W%s {Gsay '%s'{x\n\r", speaker, target_name);
	buf1[0] = UPPER (buf1[0]);
...
void spell_confusion( int sn, int level, CHAR_DATA *ch, void *vo, int target )
    ...
    { /* target is unused but required for ROM 2.4 compilation, not req'd
...
on 2.3 */
...

       {
          ...
          if ( victim == ch )
             ...
             act( "$N is confused enough as it is!", ch, NULL, victim,
